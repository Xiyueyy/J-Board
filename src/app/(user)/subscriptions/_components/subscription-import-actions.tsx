...
export function SubscriptionImportActions({
  ...
  return (
    <div className="min-w-0 rounded-2xl border border-border/70 bg-background/70 p-3 backdrop-blur">
      <div className={cn("flex min-w-0 flex-col gap-3", compact ? "" : "lg:flex-row lg:items-center lg:justify-between")}>
        <div className="min-w-0 space-y-1">
          ...
          <p className="break-all font-mono text-xs leading-5 text-foreground/80 sm:truncate">{genericUrl}</p>
        ...
        <div className="grid gap-2 sm:grid-cols-2 lg:flex lg:shrink-0 lg:flex-wrap lg:justify-end">
          <CopyButton text={clashUrl} label="复制 Clash" className="w-full" />
          <CopyButton text={genericUrl} label="复制通用" className="w-full" />
        </div>
      </div>
    </div>
  );
