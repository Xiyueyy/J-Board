...
```bash
curl -fsSL https://raw.githubusercontent.com/Xiyueyy/J-Board/main/scripts/install-jboard-agent.sh | SERVER_URL=https://panel.example.com AUTH_TOKEN=你的Token bash
```

如果需要指定 Agent Release：

```bash
curl -fsSL https://raw.githubusercontent.com/Xiyueyy/J-Board/main/scripts/install-jboard-agent.sh | AGENT_TAG=v3.0.6 SERVER_URL=https://panel.example.com AUTH_TOKEN=你的Token bash
```

## 升级

```bash
curl -fsSL https://raw.githubusercontent.com/Xiyueyy/J-Board/main/scripts/upgrade-jboard-agent.sh | bash
```
...
```text
[agent] jboard-agent v3.0.6 starting in probe-only mode
```
